...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    // 'deocoincore-lib': 'https://github.com/deocoinproject/deocoincore-lib.git#master',
    // 'deocoincore-node': 'https://github.com/deocoinproject/deocoincore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/deocoincore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the deocoincore-node directory and configuration

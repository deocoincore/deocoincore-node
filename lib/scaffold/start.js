...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for deocoind is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and deocoincore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed deocoincore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './deocoincore-node.json');
  ...
  node.on('ready', function() {
    log.info('deocoincore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in deocoincore-node services directory
    ...
    if (servicePackage.deocoincoreNode) {
      serviceModule = service.name + '/' + servicePackage.deocoincoreNode;
    }

...
var bitcore = require('deocoincore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var deocoinRPC = require('deocoind-rpc');
...
/**
 * Provides a friendly event driven API to deocoind in Node.js. Manages starting and
 * stopping deocoind as a child process for application support, as well as connecting
 * to multiple deocoind processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function deocoin(options) {
  if (!(this instanceof deocoin)) {
    return new deocoin(options);
  }
...
util.inherits(deocoin, Service);

deocoin.dependencies = [];

deocoin.DEFAULT_MAX_TXIDS = 1000;
deocoin.DEFAULT_MAX_HISTORY = 50;
deocoin.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
deocoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
deocoin.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
deocoin.DEFAULT_SPAWN_RESTART_TIME = 5000;
deocoin.DEFAULT_SPAWN_STOP_TIME = 10000;
deocoin.DEFAULT_TRY_ALL_INTERVAL = 1000;
deocoin.DEFAULT_REINDEX_INTERVAL = 10000;
deocoin.DEFAULT_START_RETRY_INTERVAL = 5000;
deocoin.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
deocoin.DEFAULT_TRANSACTION_CONCURRENCY = 5;
deocoin.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
deocoin.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || deocoin.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || deocoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || deocoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || deocoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || deocoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || deocoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || deocoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || deocoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || deocoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || deocoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

deocoin.prototype._initCaches = function() {
...
deocoin.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
deocoin.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
deocoin.prototype.getPublishEvents = function() {
  return [
    {
...
deocoin.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'deocoind/' + name, 'total:', this.subscriptions[name].length);
};

deocoin.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'deocoind/' + name, 'total:', this.subscriptions[name].length);
};


deocoin.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'deocoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

deocoin.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'deocoind/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

deocoin.prototype.unsubscribeBalanceAll = function(emitter) {
...
deocoin.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'deocoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};

deocoin.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'deocoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
deocoin.prototype.unsubscribeAddressAll = function(emitter) {
...
deocoin.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = deocoin.DEFAULT_CONFIG_SETTINGS;
...
deocoin.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
deocoin.prototype._expandRelativeDatadir = function() {
...
deocoin.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in deocoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in deocoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in deocoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './deocoin.conf');

  log.info('Using deocoin config file:', configPath);
...
deocoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of deocoincore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of deocoincore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of deocoincore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to deocoind from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from deocoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from deocoind. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that deocoind is undergoing a reindex. ' +
             ...
             'of deocoincore-node services will start.');
    node._reindex = true;
  }
};

deocoin.prototype._resetCaches = function() {
...
deocoin.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
deocoin.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

deocoin.prototype._initChain = function(callback) {
  var self = this;
...
deocoin.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 8332
  };
...
deocoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/deocoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/deocoin.conf';
    }
  }
  return networkPath;
};

deocoin.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
deocoin.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('deocoind/hashblock', message.toString('hex'));
    }
  }

};

deocoin.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
deocoin.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('deocoin Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
deocoin.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
deocoin.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
deocoin.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('deocoind/addressbalance', {
                    address: address,
                    txid: txid,
...
deocoin.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('deocoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
deocoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('deocoind/rawtransaction', message.toString('hex'));
    }
...
deocoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || deocoin.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

deocoin.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
deocoin.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
deocoin.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('deocoin Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || deocoin.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

deocoin.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
deocoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/deocoind.pid';
...
deocoin.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting deocoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('deocoin process unexpectedly exited with code:', code);
        log.warn('Restarting deocoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('deocoin process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn deocoind.'));
      }
...
deocoin.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to deocoind.'));
    }
 ...
 */
deocoin.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('deocoin configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
deocoin.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
deocoin.prototype.syncPercentage = function(callback) {
  var self = this;
...
deocoin.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, deocoincore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
deocoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
deocoin.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
deocoin.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
deocoin.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in deocoincore is most recent to less recent
 ...
 * @param {Function} callback
 */
deocoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
deocoin.prototype._getConfirmationsDetail = function(transaction) {
...
deocoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
deocoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
deocoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A deocoincoin transaction id
 * @param {Function} callback
 */
deocoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
deocoin.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
deocoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
deocoin.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
deocoin.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
deocoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
deocoin.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
deocoin.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
deocoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a deocoincore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
deocoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getInfo = function(callback) {
...
deocoin.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('deocoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('deocoind process did not exit'));
      }
 ...
 * @param {Array} addresses - A array of deocoin addresses to filter
 * @param {Function} callback
 */
deocoin.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
deocoin.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
deocoin.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
deocoin.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
deocoin.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
deocoin.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
deocoin.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
deocoin.prototype.getStakingInfo = function(callback) {
    var self = this,

...
var Transaction = deocoincore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/deocoin.conf')));
var deocoinService = proxyquire('../../lib/services/deocoind', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultdeocoinConf = fs.readFileSync(path.resolve(__dirname, '../data/default.deocoin.conf'), 'utf8');

describe('deocoin Service', function() {
	...
	var baseConfig = {
		node: {
			network: deocoincore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var deocoind = new deocoinService(baseConfig);
			should.exist(deocoind);
		});
		it('will create an instance without `new`', function() {
			var deocoind = deocoinService(baseConfig);
			should.exist(deocoind);
		});
		it('will init caches', function() {
			var deocoind = new deocoinService(baseConfig);

			should.exist(deocoind.utxosCache);
			should.exist(deocoind.txidsCache);
			should.exist(deocoind.balanceCache);
			should.exist(deocoind.summaryCache);
			should.exist(deocoind.transactionDetailedCache);
			should.exist(deocoind.accountInfo);

			should.exist(deocoind.transactionCache);
			should.exist(deocoind.rawTransactionCache);
			should.exist(deocoind.rawJsonTransactionCache);
			should.exist(deocoind.blockCache);
			should.exist(deocoind.blockJsonCache);
			should.exist(deocoind.blockSubsidyCache);
			should.exist(deocoind.rawBlockCache);
			should.exist(deocoind.blockHeaderCache);
			should.exist(deocoind.zmqKnownTransactions);
			should.exist(deocoind.zmqKnownBlocks);
			should.exist(deocoind.lastTip);
			should.exist(deocoind.lastTipTimeout);
			should.equal(deocoind.dgpInfoCache, null, 'should be null');
			should.equal(deocoind.miningInfoCache, null, 'should be null');
			should.equal(deocoind.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(deocoind.maxTxids, 1000);
			should.equal(deocoind.maxTransactionHistory, 50);
			should.equal(deocoind.maxAddressesQuery, 10000);
			should.equal(deocoind.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(deocoind.spawnRestartTime, 5000);
			should.equal(deocoind.spawnStopTime, 10000);

			// try all interval
			should.equal(deocoind.tryAllInterval, 1000);
			should.equal(deocoind.startRetryInterval, 5000);

			// rpc limits
			should.equal(deocoind.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(deocoind.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var deocoind = new deocoinService(baseConfig);
			deocoind.nodes.should.deep.equal([]);
			deocoind.nodesIndex.should.equal(0);
			deocoind.nodes.push({ client: sinon.stub() });
			should.exist(deocoind.client);
		});
		it('will set subscriptions', function() {
			var deocoind = new deocoinService(baseConfig);
			deocoind.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var deocoind = new deocoinService(baseConfig);
			deocoind._initDefaults({ transactionConcurrency: 10 });
			deocoind.transactionConcurrency.should.equal(10);
			deocoind._initDefaults({});
			deocoind.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			deocoinService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var deocoind = new deocoinService(baseConfig);
			var methods = deocoind.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var deocoind = new deocoinService(baseConfig);
			var events = deocoind.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('deocoind/rawtransaction');
			events[0].scope.should.equal(deocoind);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('deocoind/hashblock');
			events[1].scope.should.equal(deocoind);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('deocoind/addresstxid');
			events[2].scope.should.equal(deocoind);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('deocoind/addressbalance');
			events[3].scope.should.equal(deocoind);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var deocoind = new deocoinService(baseConfig);
			deocoind.subscribe = sinon.stub();
			deocoind.unsubscribe = sinon.stub();
			var events = deocoind.getPublishEvents();

			events[0].subscribe('test');
			deocoind.subscribe.args[0][0].should.equal('rawtransaction');
			deocoind.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			deocoind.unsubscribe.args[0][0].should.equal('rawtransaction');
			deocoind.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			deocoind.subscribe.args[1][0].should.equal('hashblock');
			deocoind.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			deocoind.unsubscribe.args[1][0].should.equal('hashblock');
			deocoind.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var deocoind = new deocoinService(baseConfig);
			var emitter = {};
			deocoind.subscribe('hashblock', emitter);
			deocoind.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			deocoind.subscribe('rawtransaction', emitter2);
			deocoind.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			deocoind.subscribe('hashblock', emitter1);
			deocoind.subscribe('hashblock', emitter2);
			deocoind.subscribe('hashblock', emitter3);
			deocoind.subscribe('hashblock', emitter4);
			deocoind.subscribe('hashblock', emitter5);
			deocoind.subscriptions.hashblock.length.should.equal(5);

			deocoind.unsubscribe('hashblock', emitter3);
			deocoind.subscriptions.hashblock.length.should.equal(4);
			deocoind.subscriptions.hashblock[0].should.equal(emitter1);
			deocoind.subscriptions.hashblock[1].should.equal(emitter2);
			deocoind.subscriptions.hashblock[2].should.equal(emitter4);
			deocoind.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var deocoind = new deocoinService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			deocoind.subscriptions.hashblock = [emitter1];
			deocoind.unsubscribe('hashblock', emitter3);
			deocoind.subscriptions.hashblock.length.should.equal(1);
			deocoind.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			deocoind.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(deocoind.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var deocoind = new deocoinService(baseConfig);
			var emitter = new EventEmitter();
			deocoind.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var deocoind = new deocoinService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			deocoind.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			deocoind.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			deocoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			deocoind.unsubscribeAddress(emitter1);
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			deocoind.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			deocoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			deocoind.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			deocoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(deocoind, 'unsubscribeAddressAll');
			deocoind.unsubscribeAddress(emitter1);
			deocoind.unsubscribeAddressAll.callCount.should.equal(1);
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			deocoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			deocoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			deocoind.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			deocoind.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			deocoind.unsubscribeAddressAll(emitter1);
			deocoind.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			deocoind.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			deocoind.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(deocoind.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var deocoind = new deocoinService(baseConfig);
			var config = deocoind._getDefaultConfig();
			config.should.equal(defaultdeocoinConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a deocoin.conf file', function() {
			var Testdeocoin = proxyquire('../../lib/services/deocoind', {
				fs: {
					readFileSync: readFileSync,
			...
			var deocoind = new Testdeocoin(baseConfig);
			deocoind.options.spawn.datadir = '/tmp/.deocoin';

			sinon.spy(deocoind, '_expandRelativeDatadir');
			sinon.spy(deocoind, '_checkConfigIndexes');
			sinon.spy(deocoind, '_getDefaultConf')
			sinon.spy(deocoind, '_getNetworkConfigPath');
			sinon.spy(deocoind, '_parseBitcoinConf');

			var node = {};
			deocoind._loadSpawnConfiguration(node);

			deocoind._expandRelativeDatadir.callCount.should.equal(1);
			deocoind._checkConfigIndexes.callCount.should.equal(1);
			deocoind._getDefaultConf.callCount.should.equal(1);
			deocoind._getNetworkConfigPath.callCount.should.equal(1);
			deocoind._parseBitcoinConf.callCount.should.equal(2);

			should.exist(deocoind.spawn.config);
			deocoind.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: deocoincore.Networks.testnet,
					configPath: '/tmp/.deocoincore/deocoincore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			var deocoind = new Testdeocoin(config);
			...
			deocoind.options.spawn.datadir = './data';
			var node = {};
			deocoind._loadSpawnConfiguration(node);
			...
			deocoind.options.spawn.datadir.should.equal('/tmp/.deocoincore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var Testdeocoin = proxyquire('../../lib/services/deocoind', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/baddeocoin.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var deocoind = new Testdeocoin(baseConfig);
			(function() {
				deocoind._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(deocoincore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the deocoin.conf file', function() {
			var deocoind = new deocoinService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				deocoind._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the deocoin.conf file', function() {
			var deocoind = new deocoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the deocoin.conf file', function() {
			var deocoind = new deocoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the deocoin.conf file', function() {
			var deocoind = new deocoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the deocoin.conf file', function() {
			var deocoind = new deocoinService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				deocoind.transactionDetailedCache.set(keys[i], {});
				deocoind.utxosCache.set(keys[i], {});
				deocoind.accountInfo.set(keys[i], {});
				deocoind.txidsCache.set(keys[i], {});
				deocoind.balanceCache.set(keys[i], {});
				deocoind.summaryCache.set(keys[i], {});
				deocoind.blockOverviewCache.set(keys[i], {});
				deocoind.blockJsonCache.set(keys[i], {});
			}
			deocoind._resetCaches();
			should.equal(deocoind.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(deocoind.utxosCache.get(keys[0]), undefined);
			should.equal(deocoind.accountInfo.get(keys[0]), undefined);
			should.equal(deocoind.txidsCache.get(keys[0]), undefined);
			should.equal(deocoind.balanceCache.get(keys[0]), undefined);
			should.equal(deocoind.summaryCache.get(keys[0]), undefined);
			should.equal(deocoind.blockOverviewCache.get(keys[0]), undefined);
			should.equal(deocoind.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			deocoind._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				deocoind.nodes[0].client.getInfo.callCount.should.equal(1);
				deocoind.nodes[1].client.getInfo.callCount.should.equal(1);
				deocoind.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			deocoind._tryAllClients(function(client, next) {
				...
				deocoind.nodes[1].client.getInfo.callCount.should.equal(1);
				deocoind.nodes[2].client.getInfo.callCount.should.equal(1);
				deocoind.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert deocoind-rpc error object into JavaScript error', function() {
			var deocoind = new deocoinService(baseConfig);
			var error = deocoind._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			deocoind.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			deocoind.nodes.push({
				client: {
			...
			deocoind._initChain(function() {
				log.info.callCount.should.equal(1);
				deocoind.getRawBlock.callCount.should.equal(1);
				deocoind.getRawBlock.args[0][0].should.equal('genesishash');
				deocoind.height.should.equal(5000);
				deocoind.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		it('it will handle error from getBestBlockHash', function(done) {
			...
			deocoind._initChain(function(err) {
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			deocoind.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			deocoind._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			deocoincore.Networks.disableRegtest();
			baseConfig.node.network = deocoincore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: deocoincore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var deocoind = new deocoinService(config);
			deocoind._getDefaultConf().rpcport.should.equal(8332);
		});
		it('will get default rpc port for testnet', function() {
			...
			var deocoind = new deocoinService(config);
			deocoind._getDefaultConf().rpcport.should.equal(18332);
		});
		it('will get default rpc port for regtest', function() {
			deocoincore.Networks.enableRegtest();
			var config = {
				node: {
					network: deocoincore.Networks.testnet
				},
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var deocoind = new deocoinService(config);
			should.equal(deocoind._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var deocoind = new deocoinService(config);
			deocoind._getNetworkConfigPath().should.equal('testnet3/deocoin.conf');
		});
		it('will get default rpc port for regtest', function() {
			deocoincore.Networks.enableRegtest();
			var config = {
			...
			var deocoind = new deocoinService(config);
			deocoind._getNetworkConfigPath().should.equal('regtest/deocoin.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var deocoind = new deocoinService(baseConfig);
			deocoind.node.network = deocoincore.Networks.testnet;
			deocoind._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var deocoind = new deocoinService(baseConfig);
			deocoind.node.network = deocoincore.Networks.testnet;
			deocoincore.Networks.enableRegtest();
			deocoind._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var deocoind = new deocoinService(baseConfig);
			deocoind.node.network = deocoincore.Networks.livenet;
			deocoincore.Networks.enableRegtest();
			should.equal(deocoind._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			deocoind._rapidProtectedUpdateTip = sinon.stub();
			deocoind.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			deocoind._zmqBlockHandler(node, message);
			deocoind._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			deocoind._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(deocoind, 'emit');
			deocoind.on('block', function(block) {
				block.should.equal(message);
				deocoind.emit.callCount.should.equal(1);
				deocoind._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			deocoind._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			deocoind._zmqBlockHandler(node, message);
			deocoind._rapidProtectedUpdateTip.callCount.should.equal(1);
			deocoind._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			deocoind._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(deocoind, 'emit');
			var emitter = new EventEmitter();
			deocoind.subscriptions.hashblock.push(emitter);
			emitter.on('deocoind/hashblock', function(blockHash) {
				deocoind._rapidProtectedUpdateTip.callCount.should.equal(1);
				deocoind.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				deocoind._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.syncPercentage = sinon.stub();
			sinon.spy(deocoind, '_resetCaches');
			deocoind.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				deocoind._resetCaches.callCount.should.equal(1);
			...
			deocoind._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			deocoind.on('synced', function() {
				deocoind.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			deocoind.on('synced', function() {
				deocoind.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			deocoind.on('error', function(err) {
				log.error.callCount.should.equal(1);
				deocoind.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.syncPercentage = sinon.stub();
			deocoind._resetCaches = sinon.stub();
			deocoind.on('tip', function(height) {
				deocoind._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				deocoind.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using deocoincore.Transaction', function() {
			var deocoind = new deocoinService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = deocoincore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(deocoincore.Networks.testnet);
			var outputAddress = deocoincore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = deocoincore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: deocoincore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = deocoind._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			var deocoind = new deocoinService(baseConfig);
			var tx = deocoincore.Transaction();
			tx.addInput(deocoincore.Transaction.Input({
				prevTxId: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				script: deocoincore.Script('OP_TRUE'),
				outputIndex: 1,
				output: {
					script: deocoincore.Script('OP_TRUE'),
					satoshis: 5000000000
			...
			tx.addOutput(deocoincore.Transaction.Output({
				script: deocoincore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = deocoind._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var deocoind = new deocoinService(baseConfig);
			var tx = deocoincore.Transaction();
			var address = deocoincore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(deocoincore.Transaction.Output({
				script: deocoincore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(deocoincore.Transaction.Output({
				script: deocoincore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			deocoind._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			deocoind.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('deocoind/addresstxid', function(data) {
			...
			deocoind._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			deocoind.subscriptions.rawtransaction.push(emitter);
			emitter.on('deocoind/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			deocoind._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(deocoind, 'emit');
			deocoind.subscriptions.rawtransaction.push(emitter);
			emitter.on('deocoind/rawtransaction', function() {
				done();
			});
			var node = {};
			deocoind._zmqTransactionHandler(node, expectedBuffer);
		...
		it('will emit "tx" event', function(done) {
			var deocoind = new deocoinService(baseConfig);
			var expectedBuffer = new Buffer(txhex, 'hex');
			deocoind.on('tx', function(buffer) {
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind._updateTip = sinon.stub();
			deocoind._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			deocoind.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			deocoind._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				deocoind._updateTip.callCount.should.equal(11);
				deocoind._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			deocoind._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				deocoind._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			deocoind._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				deocoind._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			deocoind._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				deocoind._zmqBlockHandler.callCount.should.equal(0);
				deocoind._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var deocoind = new deocoinService(baseConfig);
			var node = {};
			deocoind._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			deocoind._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			deocoind._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			deocoind._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
		...
		it('will set height and emit tip', function(done) {
			...
			deocoind.on('tip', function(height) {
				...
				deocoind.height.should.equal(100);
				done();
			});
			deocoind._loadTipFromNode(node, function(err) {
				if (err) {
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestdeocoinService = proxyquire('../../lib/services/deocoind', {
				fs: {
					readFile: readFile
				}
			});
			var deocoind = new TestdeocoinService(baseConfig);
			deocoind.spawnStopTime = 1;
			deocoind._process = {};
			deocoind._process.kill = sinon.stub();
			deocoind._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				deocoind._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			deocoind._process.kill = sinon.stub().throws(error2);
			deocoind._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind._loadSpawnConfiguration = sinon.stub();
			deocoind._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			deocoind._spawnChildProcess(function(err) {
				deocoind._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var deocoind = new deocoinService(baseConfig);
			deocoind._loadSpawnConfiguration = sinon.stub();
			deocoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			deocoind._spawnChildProcess(function(err) {
				deocoind._loadSpawnConfiguration.callCount.should.equal(1);
				deocoind._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var deocoind = new TestdeocoinService(config);
			deocoind.spawn = {};
			deocoind._loadSpawnConfiguration = sinon.stub();
			deocoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			deocoind.node.stopping = true;
			deocoind._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			deocoind._loadSpawnConfiguration = sinon.stub();
			deocoind.spawn = {};
			deocoind.spawn.exec = 'testexec';
			deocoind.spawn.configPath = 'testdir/deocoin.conf';
			deocoind.spawn.datadir = 'testdir';
			deocoind.spawn.config = {};
			deocoind.spawn.config.rpcport = 20001;
			deocoind.spawn.config.rpcuser = 'deocoin';
			deocoind.spawn.config.rpcpassword = 'password';
			deocoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			deocoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			deocoind._initZmqSubSocket = sinon.stub();
			deocoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			deocoind._checkReindex = sinon.stub().callsArgWith(1, null);
			deocoind._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/deocoin.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				deocoind._loadTipFromNode.callCount.should.equal(1);
				deocoind._initZmqSubSocket.callCount.should.equal(1);
				should.exist(deocoind._initZmqSubSocket.args[0][0].client);
				deocoind._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				deocoind._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(deocoind._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn deocoind spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestdeocoinService = proxyquire('../../lib/services/deocoind', {
				fs: {
			...
			deocoind._loadSpawnConfiguration = sinon.stub();
			deocoind.spawn = {};
			deocoind.spawn.exec = 'deocoind';
			deocoind.spawn.datadir = '/tmp/deocoin';
			deocoind.spawn.configPath = '/tmp/deocoin/deocoin.conf';
			deocoind.spawn.config = {};
			deocoind.spawnRestartTime = 1;
			deocoind._loadTipFromNode = sinon.stub().callsArg(1);
			deocoind._initZmqSubSocket = sinon.stub();
			deocoind._checkReindex = sinon.stub().callsArg(1);
			deocoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			deocoind._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(deocoind, '_spawnChildProcess');
			deocoind._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						deocoind._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			deocoind._spawnChildProcess(function(err) {
				...
				deocoind._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				deocoind.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn deocoind spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestdeocoinService = proxyquire('../../lib/services/deocoind', {
				fs: {
			...
			deocoind._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						deocoind._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			deocoind.spawn.config.rpcpassword = 'password';
			deocoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			deocoind._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			deocoind._spawnChildProcess(function(err) {
				deocoind._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			deocoind._initZmqSubSocket = sinon.stub();
			deocoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			deocoind._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var deocoind = new deocoinService(config);
			deocoind.node.stopping = true;
			deocoind.startRetryInterval = 100;
			deocoind._loadTipFromNode = sinon.stub();
			deocoind._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to deocoind/);
				deocoind._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			deocoind.startRetryInterval = 1;
			var config = {};
			deocoind._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				deocoind._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			deocoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			deocoind._connectProcess(config, function(err, node) {
				should.not.exist(err);
				deocoind._loadTipFromNode.callCount.should.equal(1);
				deocoind._initZmqSubSocket.callCount.should.equal(1);
				deocoind._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.options = {};
			deocoind.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('deocoin configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			deocoind.options = {
				connect: [
					{}
				]
			...
			deocoind.start(function(err) {
				deocoind._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var deocoind = new deocoinService(baseConfig);
			var node = {};
			deocoind._initChain = sinon.stub().callsArg(0);
			deocoind._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			deocoind.options = {
				spawn: {}
			};
			deocoind.start(function(err) {
				should.not.exist(err);
				deocoind.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(deocoind, '_spawnChildProcess');
			var nodes = [{}];
			deocoind._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			deocoind.options = {
				connect: [
					{}
				]
			...
			deocoind.start(function(err) {
				should.not.exist(err);
				deocoind._initChain.callCount.should.equal(1);
				deocoind._connectProcess.callCount.should.equal(1);
				deocoind.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		it('will give error from syncPercentage', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			deocoind.isSynced(function(err) {
				should.exist(err);
				deocoind.syncPercentage.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			deocoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				deocoind.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			deocoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			deocoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			deocoind.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.syncPercentage(function(err) {
				should.exist(err);
				deocoind.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			deocoind.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				deocoind.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var deocoind = new deocoinService(baseConfig);
			var args = deocoind._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var deocoind = new deocoinService(baseConfig);
			var args = deocoind._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var deocoind = new deocoinService(baseConfig);
			sinon.spy(deocoind, '_normalizeAddressArg');
			deocoind.nodes.push({
				client: {
			...
			var options = {};
			deocoind.getAddressBalance(address, options, function(err) {
				deocoind._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			deocoind.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				deocoind.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					deocoind._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			deocoind.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = deocoind._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = deocoind._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = deocoind._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = deocoind._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				deocoind._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			deocoind.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(deocoind, '_getHeightRangeQuery');
			sinon.spy(deocoind, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			deocoind.getAddressTxids(address, options, function(err) {
				should.exist(err);
				deocoind._getHeightRangeQuery.callCount.should.equal(1);
				deocoind._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			deocoind.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				deocoind._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			deocoind.getAddressTxids(address, options, function(err, txids) {
				...
				deocoind.getAddressTxids(address, options, function(err, txids) {
					...
					deocoind._getHeightRangeQuery.callCount.should.equal(2);
					deocoind._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			deocoind.getAddressTxids(address, options, function(err, txids) {
				...
				deocoind.getAddressTxids(address, options, function(err, txids) {
					...
					deocoind._getHeightRangeQuery.callCount.should.equal(4);
					deocoind._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			deocoind.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				deocoind.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					deocoind.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						deocoind._getHeightRangeQuery.callCount.should.equal(3);
						deocoind._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var deocoind = new deocoinService(baseConfig);
			deocoind.height = 10;
			var confirmations = deocoind._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var deocoind = new deocoinService(baseConfig);
			var result = {};
			deocoind._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var deocoind = new deocoinService(baseConfig);
			var result = {};
			deocoind._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var deocoind = new deocoinService(baseConfig);
			var result = {};
			deocoind._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var deocoind = new deocoinService(baseConfig);
			var result = {};
			deocoind._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(deocoind, '_getAddressDetailsForInput');
			sinon.spy(deocoind, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = deocoind._getAddressDetailsForTransaction(tx, addresses);
			...
			deocoind._getAddressDetailsForInput.callCount.should.equal(1);
			deocoind._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var deocoind = new deocoinService(baseConfig);
			sinon.spy(deocoind, '_getConfirmationsDetail');
			deocoind.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			deocoind.height = 300;
			var addresses = {};
			deocoind._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			deocoind._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				deocoind.getDetailedTransaction.callCount.should.equal(1);
				deocoind._getAddressDetailsForTransaction.callCount.should.equal(1);
				deocoind._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var deocoind = new deocoinService(baseConfig);
			deocoind.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			deocoind._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				deocoind.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from deocoincore addresses', function() {
			var addresses = [
				deocoincore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				deocoincore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var deocoind = new deocoinService(baseConfig);
			var strings = deocoind._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var deocoind = new deocoinService(baseConfig);
			(function() {
				deocoind._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var deocoind = new deocoinService(baseConfig);
			sinon.spy(deocoind, '_normalizeAddressArg');
			deocoind.maxAddressesQuery = 100;
			deocoind.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				deocoind._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var deocoind = new deocoinService(baseConfig);
			sinon.spy(deocoind, '_normalizeAddressArg');
			sinon.spy(deocoind, '_getAddressStrings');
			deocoind.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				deocoind._normalizeAddressArg.callCount.should.equal(1);
				deocoind._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(deocoind, '_normalizeAddressArg');
			sinon.spy(deocoind, '_getAddressStrings');
			deocoind.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			deocoind.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				deocoind._normalizeAddressArg.callCount.should.equal(1);
				deocoind._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(deocoind, '_normalizeAddressArg');
			sinon.spy(deocoind, '_getAddressStrings');
			deocoind.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			deocoind._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			deocoind.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				deocoind._normalizeAddressArg.callCount.should.equal(1);
				deocoind._getAddressStrings.callCount.should.equal(1);
				deocoind.getAddressTxids.callCount.should.equal(1);
				deocoind._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(deocoind, '_normalizeAddressArg');
			sinon.spy(deocoind, '_getAddressStrings');
			sinon.spy(deocoind, 'getAddressTxids');
			deocoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			deocoind.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				deocoind._normalizeAddressArg.callCount.should.equal(1);
				deocoind._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(deocoind, '_normalizeAddressArg');
			sinon.spy(deocoind, '_getAddressStrings');
			sinon.spy(deocoind, '_paginateTxids');
			deocoind._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(deocoind, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			deocoind.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			deocoind.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				deocoind._normalizeAddressArg.callCount.should.equal(1);
				deocoind._getAddressStrings.callCount.should.equal(1);
				deocoind.getAddressTxids.callCount.should.equal(1);
				deocoind._getAddressDetailedTransaction.callCount.should.equal(2);
				deocoind._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			deocoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			deocoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			deocoind.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			deocoind.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			deocoind.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			deocoind.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			deocoind.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			deocoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			deocoind.getAddressSummary(address, options, function(err, summary) {
				deocoind._paginateTxids.callCount.should.equal(1);
				deocoind._normalizeAddressArg.callCount.should.equal(1);
				deocoind.getAddressTxids.callCount.should.equal(1);
				deocoind.getAddressBalance.callCount.should.equal(1);
				deocoind._paginateTxids.args[0][1].should.equal(0);
				deocoind._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			deocoind.getAddressSummary(address, options, function(err, summary) {
				...
				deocoind.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					deocoind._paginateTxids.callCount.should.equal(0);
					deocoind._normalizeAddressArg.callCount.should.equal(2);
					deocoind.getAddressTxids.callCount.should.equal(1);
					deocoind.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			deocoind.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				deocoind._paginateTxids.callCount.should.equal(1);
				deocoind._normalizeAddressArg.callCount.should.equal(1);
				deocoind.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			deocoind._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			deocoind.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(deocoind, '_tryAllClients');
			deocoind.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			deocoind.getRawBlock(blockhash, function(err) {
				should.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			deocoind.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			deocoind.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			deocoind.getBlock(0, function(err) {
				deocoind._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as deocoincore object from height', function(done) {
			var deocoind = new deocoinService(baseConfig);
			sinon.spy(deocoind, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			deocoind.getBlock(0, function(err, block) {
				should.not.exist(err);
				deocoind._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(deocoincore.Block);
				done();
			});
		});
		it('will getblock as deocoincore object', function(done) {
			var deocoind = new deocoinService(baseConfig);
			sinon.spy(deocoind, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			deocoind.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			deocoind.getBlock(hash, function(err, block) {
				should.not.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			deocoind.getBlock(0, function(err, block) {
				...
				deocoind.getBlock(0, function(err, block) {
					should.not.exist(err);
					deocoind._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			deocoind.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var deocoind = new deocoinService(baseConfig);
			sinon.spy(deocoind, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			deocoind.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			deocoind.getBlockHeader(10, function(err) {
				deocoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			deocoind.getBlockHeader(blockhash, function(err) {
				deocoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			deocoind.getBlockHeader(0, function(err) {
				deocoind._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			deocoind.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				deocoind._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			deocoind.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				deocoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			deocoind._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				deocoind._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			deocoind._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			deocoind._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var deocoind = new deocoinService(baseConfig);
			sinon.spy(deocoind, '_maybeGetBlockHash');
			deocoind._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			deocoind.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				deocoind._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			deocoind.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				deocoind.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					deocoind._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			deocoind.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = deocoincore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			deocoind.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			deocoind.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			deocoind.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = deocoincore.Transaction();
			(function() {
				deocoind.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getRawTransaction('txid', function(err) {
				should.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			deocoind.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getTransaction('txid', function(err) {
				should.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			deocoind.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				deocoind._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(deocoincore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			deocoind.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				deocoind._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			deocoind.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			deocoind.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			deocoind.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.node.getNetworkName = sinon.stub().returns('testnet');
			...
			deocoind.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			deocoind.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			deocoind.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				deocoind.getSubsidy.callCount.should.equal(1);
				deocoind.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			deocoind.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.spawn = {};
			deocoind.spawn.process = new EventEmitter();
			deocoind.spawn.process.kill = sinon.stub();
			deocoind.stop(done);
			deocoind.spawn.process.kill.callCount.should.equal(1);
			deocoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
			deocoind.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			deocoind.spawn.process.kill.callCount.should.equal(1);
			deocoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
			deocoind.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var deocoind = new deocoinService(baseConfig);
			deocoind.shutdownTimeout = 300;
			deocoind.spawn = {};
			deocoind.spawn.process = new EventEmitter();
			deocoind.spawn.process.kill = sinon.stub();
			deocoind.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				deocoind._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var deocoind = new deocoinService(baseConfig);
			sinon.spy(deocoind, '_normalizeAddressArg');
			sinon.spy(deocoind, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			deocoind.nodes.push({
			...
			deocoind.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				deocoind._getBalanceFromMempool.callCount.should.equal(1);
				deocoind._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			deocoind.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			deocoind.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			deocoind.getJsonBlock(0, function(err) {
				should.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			deocoind.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			deocoind.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				deocoind._tryAllClients.callCount.should.equal(2);
				deocoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			deocoind.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
				deocoind._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var deocoind = new deocoinService(baseConfig);
			var emitter = new EventEmitter();
			deocoind.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(deocoind.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var deocoind = new deocoinService(baseConfig);
			var emitter = new EventEmitter();
			deocoind.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			deocoind.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			deocoind.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			deocoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			deocoind.unsubscribeBalance(emitter1);
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			deocoind.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			deocoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var deocoind = new deocoinService(baseConfig);
			...
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			deocoind.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			deocoind.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var deocoind = new deocoinService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			deocoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(deocoind, 'unsubscribeBalanceAll');
			deocoind.unsubscribeBalance(emitter1);
			deocoind.unsubscribeBalanceAll.callCount.should.equal(1);
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			deocoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			deocoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			deocoind.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			deocoind.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			deocoind.unsubscribeBalanceAll(emitter1);
			deocoind.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			deocoind.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			deocoind.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(deocoind.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var deocoind = new deocoinService(baseConfig);
			sinon.spy(deocoind, '_notifyBalanceSubscriber');
			deocoind._getAddressesFromTransaction = sinon.stub().returns(addresses);
			deocoind.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('deocoind/addressbalance', function(data) {
				...
				deocoind._notifyBalanceSubscriber.callCount.should.equal(1);
				deocoind._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			deocoind._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('deocoind/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				deocoind.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			deocoind._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			deocoind.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getDgpInfo(function(err, result) {
				should.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getMiningInfo(function(err, result) {
				should.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			deocoind.getStakingInfo(function(err, result) {
				should.exist(err);
				deocoind._tryAllClients.callCount.should.equal(1);
